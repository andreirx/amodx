# Runbook: deploying Track CACHE (cache-1 + cache-2 + cache-3 + cache-6)

Operator-driven. Written 2026-07-27 from the three slice docs' deployment notes;
**`cache-6` added 2026-07-28**. Commits: cache-1 `d2ecffe`, cache-2 `d75f06a`,
cache-3 `918370e`; cache-6 is uncommitted at time of writing. One combined deploy is
...
**What `cache-6` adds to this deploy, and why two probes below changed meaning.** It repairs
two transport defects that were already live in production and are unrelated to caching:
CloudFront was stripping `x-revalidation-token` (so `/api/revalidate` 401'd every backend
...
| 7 | ISR purge-on-edit: edit `/p` in admin, then `curl -s https://$H/p` after a fresh CloudFront miss (or GO LIVE NOW) | fresh content — the domain-keyed purge worked. **Needs cache-2 AND cache-6**: cache-2 fixes the path, cache-6 stops the endpoint 401'ing. If it fails, check CloudWatch for `/api/revalidate` 401 before suspecting the path |
| 7b | `POST https://$H/api/revalidate` with a correct `x-revalidation-token`, then list the asset bucket | **200** (not 401 — that is cache-6 D1), AND the matching `_cache/<buildId>/$H/<path>.cache` object is gone. The 200 alone only proves the header survived transport |
...
| 11 | `curl -sI 'https://$H/_next/image?url=<asset>&w=256&q=75'` | **200 + `content-type: image/*`**. Pre-cache-6 this returns **500 `"url" parameter is required"`** — CloudFront was deleting the query string (D2). Also confirms patched sharp/libvips serving normally |
| 11b | repeat probe 11 with `w=1080` | **different bytes/length** from `w=256`. This is the one that proves `w` is in the cache key rather than merely forwarded; without it a single 200 would look like success while every width served the first-requested variant |
...
- **cache-6 alone:** its two changes are independent of each other and of the rest of the
  track, so either can be reverted on its own — policy and origin-request-policy updates are
  in-place. Note what reverting restores: the ISR purge 401 and/or the image 500. Both are
  the *pre-existing* production behaviour, so a revert is a return to broken, not to safe.

## Post-deploy follow-ups (queued)

- Mark cache-1/2/3/6 SHIPPED in their slice docs + move to docs/shipped/slices/ once the
...
- Track TEST: `test-2` is **implemented** (2026-07-28) — `cd renderer && npm run test:serving`
  ...
  origin and stays operator-run. `test-4` is the slice that covers the CloudFront cache
  policy and viewer-request Function.
  *(Corrected 2026-07-28 during `cache-6`: this line read "`test-4` … is not started".
  `test-4` was implemented on 2026-07-28 — `infra/test/amodx-stack.test.ts`, now 17 named
  assertions including `cache-6`'s `(g)` and `(h)`. Authoritative status:
  `docs/ROADMAP.md` § Track TEST.)*
