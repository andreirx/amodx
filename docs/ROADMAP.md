...
| `cache-6` | **Distribution transport hotfixes — two production-impacting defects, both pre-existing in prod, both found by live probing on 2026-07-28.** (D1) `x-revalidation-token` was absent from `RendererOriginPolicy`, so CloudFront stripped the ISR purge credential and `/api/revalidate` 401'd every backend caller — **no deployed Layer-2 purge has ever worked**, independent of the purge *key* `cache-2` fixed. (D2) `_next/image*` used the managed `CACHING_OPTIMIZED` policy with no origin request policy, so `?url&w&q` was deleted in flight and the image Lambda answered 500 `"url" parameter is required` — **image optimization broken for every tenant**, OBSERVED on staging AND prod. Both repaired in `infra/lib/renderer-hosting.ts`; both now pinned by named assertions (`(h)`, `(g)`), since in each case the defect shipped precisely because nothing asserted the list. Qualifies under the standing no-CDK-without-named-gain directive: this repairs broken deployed behavior, probe-verified. | IMPLEMENTED 2026-07-28, **revision 2** (review iteration 1's two required changes applied — documentation reconciliation only, **no code and no comment changed**, proven by `shasum` equality with revision 1's pre-mutation golden: `infra/ARCHITECTURE.md` § *Testing* corrected 15 → **17** with `(g)`/`(h)` described and contrasted against the cache-key assertions they are confused with, and the slice doc's § *Files changed* reconciled against the deterministic `git diff --stat` inventory, which had omitted `docs/runbooks/deploy-track-cache.md`. A repo-wide stale-count sweep corrected two further live claims and deliberately left the dated `OBSERVED` transcripts alone. Revision 1 applied review iteration 0's two required corrections, both documentation/comment truth fixes with no behaviour change: the withdrawn "live `sharp` exposure is zero today" claim in `docs/TECH-DEBT.md` — the image Lambda's own Function URL is `AuthType: NONE`/`Principal: "*"`, so edge stripping bounds one path and not the exposure — and the "the optimizer's entire input" overclaim in four files, rescoped to its required **query-string** inputs since `Accept` also drives format negotiation) — review + deploy pending. Infra suite 15 → **17** assertions, both mutation-checked in their own round (each failing only its target, every temporary edit reverted and proven by `shasum`). Change surface measured by template diff: **exactly two semantic deltas** (ORP headers 7→8; `ImageCachePolicy` added + the `_next/image*` `CachePolicyId`), everything else asset-hash noise from re-running the builds. Ships with the combined CACHE deploy |
