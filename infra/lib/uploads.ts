...
export class AmodxUploads extends Construct {
    ...
    constructor(scope: Construct, id: string, props?: AmodxUploadsProps) {
        super(scope, id);

        const suffix = props?.bucketSuffix ? `-${props.bucketSuffix}` : '';

        // 1. Public Storage (Images)
        this.bucket = new s3.Bucket(this, 'AssetsBucket', {
            bucketName: `amodx-assets${suffix}`.toLowerCase(), // <--- DYNAMIC
            cors: [{
        ...
        this.privateBucket = new s3.Bucket(this, 'PrivateAssetsBucket', {
            bucketName: `amodx-private${suffix}`.toLowerCase(), // <--- DYNAMIC
            cors: [{
