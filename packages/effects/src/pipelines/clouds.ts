...
import { CLOUDS_SHADER } from "../shaders/clouds.js";
import {
...
export class CloudsPipeline implements EffectPipeline {
    ...
    async init(device: GPUDevice, format: GPUTextureFormat, canvas: HTMLCanvasElement, config: PipelineConfig): Promise<void> {
        this.device = device;
        const { pipeline, bindGroupLayout } = await createFullscreenPipeline(device, CLOUDS_SHADER, format);
