...
import { BUBBLES_SHADER } from "../shaders/bubbles.js";
import {
...
export class BubblesPipeline implements EffectPipeline {
    ...
    async init(device: GPUDevice, format: GPUTextureFormat, canvas: HTMLCanvasElement, config: PipelineConfig): Promise<void> {
        this.device = device;
        const { pipeline, bindGroupLayout } = await createFullscreenPipeline(device, BUBBLES_SHADER, format);
