...
import { AURORA2_SHADER } from "../shaders/aurora2.js";
import {
...
export class Aurora2Pipeline implements EffectPipeline {
    ...
    async init(device: GPUDevice, format: GPUTextureFormat, canvas: HTMLCanvasElement, config: PipelineConfig): Promise<void> {
        this.device = device;
        const { pipeline, bindGroupLayout } = await createFullscreenPipeline(device, AURORA2_SHADER, format);
