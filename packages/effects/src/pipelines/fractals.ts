...
import { FRACTALS_SHADER } from "../shaders/fractals.js";
import {
...
export class FractalsPipeline implements EffectPipeline {
    ...
    async init(device: GPUDevice, format: GPUTextureFormat, canvas: HTMLCanvasElement, config: PipelineConfig): Promise<void> {
        this.device = device;
        const { pipeline, bindGroupLayout } = await createFullscreenPipeline(device, FRACTALS_SHADER, format);
